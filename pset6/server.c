...
char* indexes(const char* path)
{
    ...
    strcpy(_path, path);
    char* index_file_name = "index.php";

    char* php_path = malloc(strlen(_path) + strlen(index_file_name) + 1);
    strcpy(php_path, _path);
    strcat(php_path, index_file_name);
    ...
    char* html_path = malloc(strlen(_path) + strlen(index_file_name) + 1);
    strcpy(html_path, _path);
    strcat(html_path, index_file_name);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // buffer
    BYTE* buffer = NULL;
    // single unit read from stream
    int u;

    // read one unit at a time till EOF
    for (u = fgetc(file); u != EOF; u = fgetc(file))
    {
    ...
    // minimize buffer 
    *content = malloc(lob * sizeof(BYTE));
    memcpy(*content, buffer, lob);
    free(buffer);
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // get line elements (method SP request-target SP HTTP-version CRLF)
    char method[8];
    char HTTP_version[9];
    char request_target[LimitRequestLine - 17];
    
    int no_of_elements = sscanf(line, "%s %s %s\r\n", method, request_target, HTTP_version);
    // UNUSED(no_of_elements);
    
    // check that request-line is valid
    if(no_of_elements != 3 || 
        strlen(method) == 0 || strlen(request_target) == 0 || strlen(HTTP_version) == 0 || 
        line[line_length - 2] != '\r' || line[line_length - 1] != '\n')
    {
        error(400);
        return false;
    }

    // check for GET method only
    ...
    // check request-target contains " and double whitespaces
    if(strchr(request_target, 34) != NULL || strstr(line, "  ") != NULL)
    {
        error(400);
        return false;
    }

    // check HTTP-version to be HTTP/1.1 
    ...
    // get query beginning with ?
    char* _query = strchr(request_target, '?');
    size_t query_length = 0;
    if(_query != NULL)
    {
        query_length =  strlen(_query);
        if(query_length > 1 && LimitRequestLine > query_length)
            strcpy(query, ++_query);
        else
            query[0] = '\0';
    }
    ...
    // get abs_path length
    USINT abs_path_length = strlen(request_target) - query_length;
    // copy absolute-path from request-target to abs_path
    strncpy(abs_path, request_target, abs_path_length);
    // terminate abs_path with null
    abs_path[abs_path_length] = 0;
